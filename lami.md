# LTTng analyses machine interface (LAMI) v1.0
...
As such, a consumer executing an analysis which claims to produce
...
| `unit-system` | String | Name of the [unit system](#unit-system) used by this column, if the `class` property is `string`, `int`, `number`, or `bool` | No | No unit system |

Note that the `unit` and `unit-system` properties cannot coexist.

The `unit` property is a simple way to associate a unit of measurement
to the quantities outputted in a result table's column described by this
column description object. However, this property does not indicate how
to apply another unit of measurement to those quantities by multiplying
them with a factor. For example, if the `unit` property is set to `kiB`,
then the consumer cannot know how to convert the kibibyte quantities to
mebibytes or to bytes, as `kiB` is just a label. For more flexibility
regarding units of measurement, use the `unit-system` property.

The `unit-system` property indicates the name of a
[unit system](#unit-system) defined in the `unit-systems` property
of the [metadata object](#metadata-object) defining this column
description object.
...
A _unit system_ is an array of
[unit factor objects](#unit-factor-object). Each item in this array
indicates the name of a unit of measurement when a given compatible
quantity is **multiplied** by its associated factor.
...
[
  ...
  {"unit": "P/", "factor": 6},
  {"unit": "in", "factor": 1},
  {"unit": "ft", "factor": 0.0833333},
  {"unit": "yd", "factor": 0.0277778},
  {"unit": "mi", "factor": 1.5782828e-5}
]
```
...
[
  {"unit": "B", "factor": 1},
  {"unit": "kiB", "factor": 0.0009765625},
  {"unit": "MiB", "factor": 9.5367431640e-7},
  {"unit": "GiB", "factor": 9.3132257462e-10},
  {"unit": "TiB", "factor": 9.0949470177e-13}
]
```
...
A _unit factor object_ associates the name of a unit of measurement to
a factor by which a given quantity must be multiplied to be given this
unit.
...
| Property | Type | Description | Required? | Default value |
|---|---|---|---|---|
| `unit` | String | Name of the unit of measurement | Yes | N/A |
| `factor` | Number | Factor | Yes | N/A |
...
| `unit-systems` | Object mapping unit names (strings) to [unit systems](#unit-system) | No | `{}` |
...
```json
{
  ...
  "table-classes": {
    ...
    "power-usage": {
      "title": "Power usage of devices",
      "column-descriptions": [
        {"title": "Device name", "class": "string"},
        {
          "title": "Power usage (normal)",
          "class": "number",
          "unit-system": "watt"
        },
        {
